...
# Created: 07.30.2023
#
# Packer Template to create an Ubuntu Server (22.04) with k3 on Proxmox
##############################################################################
...
# VM Template
#######################################
source "proxmox-iso" "ubuntu-server-jammy-k3" {
  ...
  http_directory = "templates/jammy-2204-k3/http"
  ...
  node                 = var.node_name
  vm_id                = var.vm_id
  vm_name              = "ubuntu-server-jammy-k3"
  template_description = "# Ubuntu Server Template\n## Jammy Image 22.04 with k3 pre-installed"
  os                   = "l26"
  bios                 = "seabios"
...
build {
  ...
  name    = "ubuntu-server-jammy-k3"
  sources = ["source.proxmox-iso.ubuntu-server-jammy-k3"]
  ...
  provisioner "file" {
    source      = "templates/jammy-2204-k3/files/99-pve.cfg"
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo curl -fsSL https://prod-cdn.packages.k3s.io/repositories/isv:/kubernetes:/core:/stable:/v1.28/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg",
      "sudo echo \"deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k3s.io/core:/stable:/v1.28/deb/ /\" | sudo tee /etc/apt/sources.list.d/kubernetes.list",
      "sudo apt-get update -y",
      "sudo apt-get install -y kubeadm kubectl kubelet",
      "sudo apt-mark hold kubeadm kubectl kubelet",
      "echo -e 'net.bridge.bridge-nf-call-iptables  = 1\nnet.bridge.bridge-nf-call-ip6tables = 1\nnet.ipv4.ip_forward                 = 1' | sudo tee /etc/sysctl.d/k3s.conf",
